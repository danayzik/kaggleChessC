...
IterativePvSearcher::IterativePvSearcher(Evaluator* evaluator, bool isWhite) : Searcher(evaluator, isWhite, std::make_unique<PvMoveSorter>()) {}

Move IterativePvSearcher::getMove(chess::Board &board, int msTimeLimit) {
...
pair<int, Move> IterativePvSearcher::iterativeDeepening(chess::Board &board) {
    int newEval = 0;
    int currEval = 0;
    Move bestMove = Move();
    int alpha = NEGINF;
...
pair<int, Move> IterativePvSearcher::minimax(Board &board, int depth, int alpha, int beta, bool isMaximizing, bool inRoot) {
